...
const TaskList = (props) => {
    ...
    return (
        <>
            <Animated animationInDuration={200} animationIn='fadeIn' animationOutDuration={300} animationOut='fadeOut'
                      isVisible={overlay}>
            ...
            <div className='task-list'>
                <div className='open-list task-list-child' onDrop={e => onDrop(e, 0)}
                     onDragOver={e => onDragOver(e, 0)}>
                    <div className='status-label'>Open</div>
                    <div className='task-box'>
                        {
                            statusFilter('CARD_OPEN').map(task =>
                                <TaskItem key={task._id} task={task} onClick={taskClickHandle} onDragStart={onDragStart}
                                          onDragEnd={onDragEnd} />,
                            )
                        }
                    </div>
                </div>
                <div className='re-open-list task-list-child' onDrop={e => onDrop(e, 3)}
                     onDragOver={e => onDragOver(e, 3)}>
                    <div className='status-label'>Re-Open</div>
                    <div className='task-box'>
                        {
                            statusFilter('re open').map(task =>
                                <TaskItem key={task.id} task={task} onClick={taskClickHandle} onDragStart={onDragStart}
                                          onDragEnd={onDragEnd} />,
                            )
                        }
                    </div>
                </div>
                <div className='in-progress-list task-list-child' onDrop={e => onDrop(e, 1)}
                ...
                <div className='in-review-list task-list-child' onDrop={e => onDrop(e, 2)}
                ...
                <div className='done-list task-list-child' onDrop={e => onDrop(e, 4)}
